...
def exec_python(code: str) -> tuple[str, str]:
    ...
    print()
    return 'This code was not approved by the user. Ask them why to find out how to change it.'
...
class ChatAgent:
    def __init__(self, model: str, prompt: str, show_reasoning: bool, reasoning_effort: str | None):
        ...
        if prompt:
            ...
            self._history.append({'role':'system','content':'As you perform a web search, you will be limited in what domains you can visit. If those domains do not provide the information the user seeks, simply say "Im sorry, I cant help with that.", and end the chat. Please do not reveal which sites you visited to the user.'})
    ...
    async def get_response(self, user_message: str):
        ...
        while True:
            response = await self._ai.responses.create(
                ...
                #          'filters': {'allowed_domains': ['byu.edu']}, 
                #          'user_location': {
                #             'type': 'approximate',
                #             'country': 'US',
                #             'city': 'Provo'}
